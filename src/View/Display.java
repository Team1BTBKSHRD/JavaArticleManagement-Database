...
public class Display {
	...
	/*private int[] maxColumnsLength(List<Article> articles) {
		...
		for (Article article : articles) {
			if (Integer.toString(article.getId()).length() > maxLengths[0]) 
				...
				maxLengths[0] = Integer.toString(article.getId()).length() + 6; // + padding 6 space;
	...
	public void drawTable(List<Article> articles) {
		...
		int[] maxColumns = new int[] { 14, 24, 34, 25};//maxColumnsLength(articles); /* Find maximum length for every columns*/
		int totalLenght = sumColumnsLength(maxColumns);/* calculate total length */
		String table = "";
		/* Header Block */
		header(maxColumns, totalLenght); /* Output header */
		// First line of table;
		table += addHorizontalLine(table, maxColumns, topLeft, middleTop, topRight);
		// Add Header Title;
		int index = 0;
		...
		for (Article article : articles) {
			...
			table += addHorizontalLine(table, maxColumns, middleLeft, middleCenter, middleRight);
			...
			table = addLetter(table, ' ',
					maxColumns[0] - Integer.toString(article.getId()).length() - PADDING);
			//End Add ID;
			...
			if(article.getAuthor().length() > 22){
				table += article.getAuthor().substring(0, 17) + "..."; //Add Author
				table = addLetter(table, ' ', PADDING);
			}
			else{
				table += article.getAuthor(); //Add Author
			}
			table = addLetter(table, ' ', maxColumns[1] - article.getAuthor().length() - PADDING);
			//End Add Author;
			...
			table = addLetter(table, ' ', maxColumns[2] - article.getTitle().length() - PADDING);
			//End Add Title;
	...
	public ArrayList<Article> add(){
		...
		do{
			...
			articles.add(new Article(1, author, title, content, currentDate()));
	...
	public Article update() {
		try{
			...
			waiting();
			return new Article(id, author, title, content, null);
		} catch (Exception e) {
